...
var _require4 = require(`./index`),
    ...
    trackSubObjectsToRootNodeId = _require4.trackSubObjectsToRootNodeId;
...
actions.createPage = function (page, plugin, traceId) {
  ...
  if (!page.component || !path.isAbsolute(page.component)) {
    var _message = `${name} must set the absolute path to the page component when create creating a page`;
    // Don't log out when testing
    if (process.env.NODE_ENV !== `test`) {
  ...
  if (!internalPage.component.includes(`/.cache/`)) {
    ...
    if (!notEmpty || !includesDefaultExport) {
      ...
      console.log(``);
      console.log(``);
      console.log(`The page component at "${relativePath}" didn't pass validation`);
